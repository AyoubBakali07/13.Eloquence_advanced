        ...
        <div class="card-header d-flex pb-0 pt-3"> 
                ...
                <form method="GET" action="{{ route('articles.index') }}" class="d-flex mb-3 "> 
                    <div class="form-group  "> 
                        <input type="text" name="search" id="search" class="form-control " value="{{ request('search') }}" placeholder="{{ __('message.search_article') }}"> 
                    </div> 
                    <div class="form-group  "> 
                    <button type="submit" class="btn btn-primary mx-3">{{__('message.search_article')}}</button> 
                    </div> 
                </form> 
                ...
                <form method="GET" action="{{ route('articles.index') }}" class="d-flex mb-3 mx-3"> 
                    ...
                    <div class="form-group mr-2"> 
                        <select name="tag" id="tag" class="form-control mx-2"> 
                            <option value="">{{ __('message.all_tags') }}</option> 
                            @foreach($tags as $tag) 
                    ...
                    <button type="submit" class="btn btn-primary mx-3">{{ __('message.filter') }}</button> 
                </form> 

        </div> 
        <!-- /.card-header --> 
         <div class="d-flex justify-content-between mx-3 mt-3"> 
             <h3 class="card-title my-0">{{__('message.article_list') }}</h3> 
              
            <a href="{{route('articles.create')}}" class="btn btn-success">{{ __('message.add_article') }}</a>   
         </div> 
            ...
            <table class="table table-bordered"> 
                <thead> 
                    <tr> 
                        ...
                        <th>{{ __('message.creation_date') }}</th> 
